...
export const validatePackageForm = (
  ...
  const errors: packageFormErrorState = { packages: [] };
  ...
  if (!packages || packages.packages.length === 0) {
    ...
    packages.packages.forEach((pkg) => {
      ...
      if (!pkg.payment?.trim()) pkgErrors.payment = "Payment is required";
      if (!pkg.deliveryTime?.trim())
        pkgErrors.deliveryTime = "Delivery time is required";
      if (!pkg.validity?.trim()) pkgErrors.validity = "Validity is required";
