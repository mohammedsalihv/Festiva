            ...
            <tbody>
              {clients.map((client, index) => (
                <tr key={index} className="border-t">
                  ...
                  <td className={`py-2 ${client.change.startsWith("+") ? "text-green-500" : "text-red-500"}`}>{client.change}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
const HostDashboard = () => {
    const [activeTab, setActiveTab] = useState<"CashFlow" | "ProfitLoss">("CashFlow");
    ...
    return (
      <div className="min-h-screen bg-gray-100 p-4 md:p-8">
        ...
        <div className="flex space-x-4 mb-6">
          ...
          <button
            className={`px-4 py-2 rounded-lg ${activeTab === "ProfitLoss" ? "bg-blue-500 text-white" : "bg-gray-200"}`}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {/* KPI Cards */}
          <KPICard title="Net Income" value="$1.2M" change="+12%" changeColor="text-green-500" />
          <KPICard title="ARR" value="$3.5M" change="-3%" changeColor="text-red-500" />
          <KPICard title="MRR" value="$290K" change="+8%" changeColor="text-green-500" />
