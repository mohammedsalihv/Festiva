...
const StudioForm = () => {
  ...
  const [errors, setErrors] = useState<studioFormErrorState>({});
  ...
  const handleAddTimeSlot = () => {
    ...
    if (studioForm.timeSlots?.includes(newSlot)) {
  ...
  const handleAddDate = () => {
    ...
    if (!studioForm.availableDates?.includes(dateInput)) {
      setStudioForm((prev) => ({
        ...prev,
  ...
  const handleSubmit = () => {
    ...
    const { isValid, errors: validationErrors } = validateStudioForm(studioForm);
          ...
          <form className="space-y-5 py-3">
            <div className="space-y-2">
              ...
              <Input
                ...
                className={`${
                  errors.studioName ? "border-red-600" : "border-black"
                }`}
              />
              {errors.studioName && (
                <p className="text-red-600 text-xs mt-1">{errors.studioName}</p>
              )}
            </div>
              ...
              <div className="space-y-2">
                <label className="block text-sm font-medium">
                 ...
                 <Input
                  type="text"
                  name="terms"
                  value={studioForm.terms}
                  onChange={handleChange}
                  className={`${
                    errors.terms ? "border-red-600" : "border-black"
                  }`}
                />
                {errors.terms && (
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-col gap-4 w-full">
                ...
                {studioForm.timeSlots.length > 0 && (
                  <div className="mt-2 flex flex-wrap gap-2">
                    {studioForm.timeSlots?.map((slot) => (
                      <div
                        key={slot}
            ...
            <div className="space-y-2">
              ...
              {studioForm.availableDates.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-2 ">
                  {studioForm.availableDates.map((date) => (
                    <div
                      key={date}
                      className="bg-gray-200 px-3 py-1 rounded-full flex items-center gap-2"
                    >
