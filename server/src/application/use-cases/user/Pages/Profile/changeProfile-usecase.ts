...
export class UserProfile {
  ...
  async execute(userId: string, image: Express.Multer.File): Promise<IUser> {
    ...
    if (!allowedMimeTypes.includes(image.mimetype)) {
      throw new CustomError("Invalid file type. Only JPEG, PNG, and GIF are allowed.", 400);
    }
    const imageName = image.filename;
    const imageUrl = `/uploads/singleImages/${imageName}`;

    const response = await this.UserProfileRepository.changeProfile(userId, imageUrl);
